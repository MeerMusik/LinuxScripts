...
# Hint: 1: Ninja expect the RPATH set exactly as it is below! If you use the Alternative commands, use: -Wl,-rpath=\$\$\$$ORIGIN 2: Do not set a C Standard Version - somehow it never works for me and the build will fail. Maybe this will get fixed in later Versons of GCC and or CMake.
# Hint: 2: The CMake Build File sets -std=c++17 very late in the List of Flags and overwrites the -std=c++20 Value. I am too lazy to write a 'SED' or similar Function to help with that. Feel free to do so yourself.
